...
FROM awrhistosstat_rs AS metrics
    INNER JOIN dbinstances AS dbinstances
            ON TRIM(metrics.PKEY) = TRIM(dbinstances.pkey)                                
...
FROM (
SELECT pkey, dbid, instance_number, hour,
PERCENTILE_CONT( 0.95) WITHIN GROUP ( ORDER BY DB_CPU_PERC95 ) AS DB_CPU_PERC95,  /*OVER(partition by pkey, dbid, instance_number, hour) */
...
SELECT tm.pkey, tm.dbid, tm.instance_number, tm.hour,
    ...
    ROUND(((CAST (DB_CPU_PERC95 AS NUMERIC)/1000000 + (CAST (BACKG_CPU_TIME_PERC95 AS NUMERIC))/1000000)*100)/ (cast(BUSY_TIME_PERC95 as numeric)/100 + cast(IDLE_TIME_PERC95 as numeric)/100),2) DB_CPU_USAGE_PERCENTAGE,
...
SELECT cpu.pkey, cpu.dbid, cpu.instance_number, cpu.hour,
    ...
    ROUND(CAST (DB_CPU_USAGE_PERCENTAGE * (CASE  WHEN (NUM_CPU_CORES_PERC95 = 0.0)  THEN  NUM_CPU_CORES_CALC_RVENG ELSE NUM_CPU_CORES_PERC95 END)/100 AS NUMERIC),2) DB_CPU_CORES,
--    ROUND(DB_CPU_USAGE_PERCENTAGE * (IF (NUM_CPU_CORES_PERC95 = 0.0, NUM_CPU_CORES_CALC_RVENG, NUM_CPU_CORES_PERC95))/100,2) DB_CPU_CORES,
    CEIL(CAST (HOST_CPU_USAGE_PERCENTAGE * (CASE WHEN (NUM_CPU_CORES_PERC95 = 0.0) THEN NUM_CPU_CORES_CALC_RVENG ELSE NUM_CPU_CORES_PERC95 END) AS NUMERIC)/100) HOST_CORES,
    NUM_CPU_CORES_PERC95,
    NUM_CPUS_PERC95,
