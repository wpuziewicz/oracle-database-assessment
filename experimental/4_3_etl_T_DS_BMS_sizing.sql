--create or replace table T_DS_BMS_sizing as
--Version 2.0.1
--BMS Sizing
...
create table T_DS_BMS_sizing as
WITH hostdetails AS (
SELECT
...
(
select DISTINCT
     dbsizing_facts.PKEY, dbinstances.host_name, dbsizing_facts.INSTANCE_NUMBER,
...
order by dbsizing_facts.PKEY, dbsizing_facts.INSTANCE_NUMBER
),
...
SELECT
     ...
     RANK() OVER (PARTITION BY PKEY
                  ORDER BY PKEY, INSTANCE_NUMBER) AS Ranking
FROM DBstorage
),
...
FROM
  vsysstat_columnar  as sysstat
--  JOIN vdbinstances as dbinstance
...
vsysstat_columnar  as sysstat    -- WAP Does not need to be this view?
  JOIN dbinstances as dbinstance
  ON sysstat.PKEY = dbinstance.PKEY AND sysstat.instance_number = dbinstance.inst_id
),
DBHostJoin as(
...
end, 0 ) as SSD_Storage_TB, --calcualte the SSD storage required
CASE
when DBIOFinalResults.iops_total_perc95 = 0 or DBIOFinalResults.iombps_total_perc95 = 0 then 0
...
COALESCE(DBIOFinalResults.iops_total_perc95, CAST(0 as NUMERIC)) iops_total_perc95,
...
LEFT JOIN optimusconfig_bms_machinesizes AS machinesizes
